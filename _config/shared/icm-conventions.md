...
| **Layer 4** — working artifacts | `meetings/CONTEXT.md` routes into `meetings/<YYYY-MM-DD-HHMM>-<topic>/` and, when an entity index exists, new records under `meetings/<scope>/<YYYY-MM-DD-HHMM>-<topic>/` (existing flat or unstamped folders stay); local `meetings/index.md` catalogs them |
...
- **Factory (Layer 3, stable):** advisor roster, `_config/shared/` rules (including `version.md`
  ...
  root `CHANGELOG.md`.
- **Product (Layer 4, per-run):** user records under `meetings/<YYYY-MM-DD-HHMM>-<topic>/`, and,
  after `_config/profile/entities/index.md` exists, new records under
  `meetings/<scope>/<YYYY-MM-DD-HHMM>-<topic>/`. Both layouts may coexist; do not relocate an
  existing folder or add a stamp to one. Tracked `meetings/CONTEXT.md` is
