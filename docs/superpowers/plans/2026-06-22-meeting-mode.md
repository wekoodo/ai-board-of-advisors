...
**Architecture:** The root `CONTEXT.md` becomes the meeting **host**. A new `_config/shared/meeting-process.md` defines the conversational loop and how convening degrades by harness capability (isolated sub-agent where supported; inline role-play otherwise — both file-based). Artifacts are written to a per-meeting `meetings/YYYY-MM-DD-topic/artifacts/`; a `transcript.md` is written only on "save." The `sessions/` tree is renamed `meetings/`, the staged `_template/` and per-advisor `output/` folders are retired, and the two examples are regenerated as meeting records.
...
- **Hybrid persistence.** Artifacts are written whenever produced; a `transcript.md` is written only when the user says "save/checkpoint this meeting."
...
└── sessions/ → meetings/                         (R) Iter 01
    ...
    ├── example-2026-emergency-fund/              (M) Iter 07   ← regenerated as a record
    └── example-2026-rental-property/             (M) Iter 07   ← regenerated as a record
```
...
- [ ] **Step 4: Update `.gitignore` glob**
  ...
  Change the user-meetings ignore line:
  ```gitignore
...
- [ ] **Step 5: Verify**
  ...
  rg -n --glob '!docs/superpowers/**' --glob '!tmp/**' -e 'sessions/' -e 'board-process' .
  ```
  Expected: **no matches.** Also confirm `meetings/` exists, `_config/shared/meeting-process.md` exists, and `.gitignore` has `meetings/????-??-??-*/`.
...
- [ ] **Step 1: Rewrite `_config/shared/meeting-process.md`**
  ...
  - **`## The Meeting Record`** — the chair creates `meetings/YYYY-MM-DD-topic/` lazily (on first artifact or on "save"), naming it from the meeting's date and a short topic slug. It holds `artifacts/` (always, when artifacts exist) and `transcript.md` (only on "save/checkpoint" — a readable capture of the conversation and decisions). No template to copy, no brief file to fill.
...
- [ ] **Step 2: Verify**
  ...
  - File contains the six sections above; "Convening Modes" describes both sub-agent and inline; "The Meeting Record" specifies `artifacts/` always + `transcript.md` on save.
  - No placeholder text.
...
- [ ] **Step 2: Verify**
  ...
  - The file has First Launch Check, Advisor Roster (10 advisors), `## Hosting a Meeting` (with convening modes + the meeting record + the save action), and Profile.
...
- [ ] **Step 1: Update `AGENTS.md`**
  ...
  In `## Folder Structure`, replace the `sessions/` three-stage description with: `meetings/` holds **records of board meetings** — each a `YYYY-MM-DD-topic/` folder with an `artifacts/` directory and an optional `transcript.md`. Replace any "three-stage / Round 1→2→3" prose with the live-meeting model: you talk to the board, the chair convenes the relevant advisors and produces artifacts as the conversation needs them. Keep `## Getting Started` ("To begin, load `CONTEXT.md`").

- [ ] **Step 2: Update `README.md` narrative**

  - Replace the "single-advisor consult vs. full board session" quick-start with one **"Talk to your board"** flow: open the project in your harness, load `CONTEXT.md`, and ask — the chair convenes the right advisors and produces artifacts you can edit; say "save this meeting" to keep a record.
  - Update any `sessions/`/staged-round wording to the meeting model.
  - Leave a clearly marked `## See it in action` stub referencing the two example meetings by name (`meetings/example-2026-emergency-fund/`, `meetings/example-2026-rental-property/`); Iter 07 fills its description after the records exist.
...
- [ ] **Step 2: Drop the `output/` rules from `.gitignore`**
  ...
  ```
  Leave the profile rules and `meetings/????-??-??-*/` intact.
...
- [ ] **Step 3: Verify**
  ...
  - `git check-ignore -v meetings/2099-01-01-x/transcript.md` → ignored by `meetings/????-??-??-*/`; `git check-ignore -v meetings/example-2026-rental-property/transcript.md` → **not** ignored (ships).
...
Replace the old staged example folders with conversational meeting records, produced by running the new protocol via sub-agents. Personas stay fictional. Reuse the existing artifact *content* where it still fits.

**Files:**
- Modify (restructure): `meetings/example-2026-emergency-fund/` → `transcript.md` (+ `artifacts/` only if one is produced)
- Modify (restructure): `meetings/example-2026-rental-property/` → `transcript.md` + `artifacts/{affordability-and-reserves-worksheet.md, rental-deal-screen-worksheet.md, due-diligence-and-location-checklist.md}`
...
- [ ] **Step 1: Salvage reusable artifact content**

  Before deleting, copy the existing rental artifacts into the new home (their content already obeys conventions):
  ```bash
  mkdir -p meetings/example-2026-rental-property/artifacts
  git mv meetings/example-2026-rental-property/02-artifacts/financial-planner/affordability-and-reserves-worksheet.md meetings/example-2026-rental-property/artifacts/affordability-and-reserves-worksheet.md
  git mv meetings/example-2026-rental-property/02-artifacts/real-estate-advisor/rental-deal-screen-worksheet.md meetings/example-2026-rental-property/artifacts/rental-deal-screen-worksheet.md
  git mv meetings/example-2026-rental-property/02-artifacts/real-estate-advisor/due-diligence-and-location-checklist.md meetings/example-2026-rental-property/artifacts/due-diligence-and-location-checklist.md
  ```

- [ ] **Step 2: Author `meetings/example-2026-rental-property/transcript.md`**

  A readable meeting transcript with the fictional couple (Priya & Marcus) as a participant. It must show, in conversational form: the chair routing to Real Estate + Financial Planner; their initial takes; the chair commissioning the two worksheets (linking the files now in `artifacts/`); **the user reviewing the affordability worksheet and requesting a change; the advisor revising it; the other advisor reacting**; a brief board synthesis; and the user saying "save this meeting." Keep figures illustrative-labeled. This is the flagship reference for the new collaborative-artifact capability. *(Recommended: produce via a sub-agent acting as the chair/advisors; the fictional user turns are authored as part of the transcript.)*

- [ ] **Step 3: Author `meetings/example-2026-emergency-fund/transcript.md`**

  A short single-advisor meeting transcript with the fictional "Daniel": chair routes to the Financial Planner, the advisor answers (target months + where to hold it + how to build), the user is satisfied, done. Demonstrates that a single-advisor consult is just a one-voice meeting. Include an `artifacts/` doc only if the transcript naturally produces one; otherwise omit the folder.

- [ ] **Step 4: Delete the old staged structure in both example dirs**
  ```bash
  git rm -r meetings/example-2026-rental-property/00-brief.md meetings/example-2026-rental-property/CONTEXT.md \
            meetings/example-2026-rental-property/01-initial-meeting meetings/example-2026-rental-property/02-artifacts \
            meetings/example-2026-rental-property/03-reconvene
  git rm -r meetings/example-2026-emergency-fund/00-brief.md meetings/example-2026-emergency-fund/CONTEXT.md \
            meetings/example-2026-emergency-fund/01-initial-meeting
  ```

- [ ] **Step 5: Finalize `README.md` `## See it in action`**

  Two bullets: the emergency-fund record as the simplest path (single advisor, one transcript); the rental-property record as the flagship (chair routes two advisors, commissions artifacts, the user co-edits one, board synthesizes, meeting saved) — link both `transcript.md` files and the rental `artifacts/`.

- [ ] **Step 6: Verify**
  - Final trees: `meetings/example-2026-rental-property/` = `transcript.md` + `artifacts/` (3 files); `meetings/example-2026-emergency-fund/` = `transcript.md` (+ optional `artifacts/`).
  - No `00-brief.md`, `CONTEXT.md`, `01-initial-meeting/`, `02-artifacts/`, `03-reconvene/` remain under `meetings/`.
  - The rental `transcript.md` shows the user requesting an artifact change and an advisor revising it. Personas fully fictional. Year-specific figures carry illustrative labels.
  - `README.md` "See it in action" links both records.

- [ ] **Step 7: Commit**
  ```bash
  git add -A
  git commit -m "feat: regenerate examples as conversational meeting records"
  ```
...
- [ ] **Step 4: `.gitignore` behavior**
  - `git check-ignore` confirms: profile data files ignored; `_config/profile/CONTEXT.md` tracked; `meetings/2099-01-01-x/` ignored; both `meetings/example-*` tracked; no `output/` rules remain.
...
| §2 Goal: meeting-first, chair-routed, living artifacts, absorb pipeline, hybrid persistence, rename | Iters 01–07 | ✓ |
...
| §5.5 Meeting record shape (`artifacts/` + `transcript.md`) | Iter 02; Iter 07 | ✓ |
